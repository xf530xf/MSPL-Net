...
from model.semseg.attention_unet import AttU_Net
...
def main():
    ...
    model_zoo = {'deeplabv3plus': DeepLabV3Plus, 'pspnet': PSPNet,'UNet':UNet}
    assert cfg['model'] in model_zoo.keys()
    model = model_zoo[cfg['model']](cfg)
    
#    model = AttU_Net(in_chns=3, class_num=cfg['nclass']) 
    if rank == 0:
...
#    optimizer = SGD(model.parameters(), cfg['lr'], momentum=0.9, weight_decay=0.0001)
    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        for i, ((imgA_x, mask_x),(imgA_u_w, imgA_u_s1, imgA_u_s2,imgA_u_s3,  ignore_mask, cutmix_box1, cutmix_box2,cutout_box3),
                (imgA_u_w_mix, imgA_u_s1_mix, imgA_u_s2_mix,imgA_u_s3_mix, ignore_mask_mix, _, _,_)) in enumerate(loader):
            
            imgA_x,  mask_x = imgA_x.cuda(), mask_x.cuda()
            imgA_u_w= imgA_u_w.cuda()
            imgA_u_s1 = imgA_u_s1.cuda()
            imgA_u_s2 = imgA_u_s2.cuda()
            imgA_u_s3=imgA_u_s3.cuda()
            ignore_mask = ignore_mask.cuda()
            cutmix_box1, cutmix_box2 = cutmix_box1.cuda(), cutmix_box2.cuda()
            imgA_u_w_mix= imgA_u_w_mix.cuda()
            imgA_u_s1_mix = imgA_u_s1_mix.cuda()
            imgA_u_s2_mix = imgA_u_s2_mix.cuda()
            imgA_u_s3_mix=imgA_u_s3_mix.cuda()
            ignore_mask_mix = ignore_mask_mix.cuda()
#################################################################################################################################
            with torch.no_grad():
                model.eval()

                pred_u_w_mix = model(imgA_u_w_mix).detach()
            ...
            imgA_u_s3[cutout_box3.unsqueeze(1).expand(imgA_u_s3.shape) == 1] = \
                imgA_u_s3_mix[cutout_box3.unsqueeze(1).expand(imgA_u_s3.shape) == 1]
        ############################################################################################################
            model.train()

            num_lb, num_ulb = imgA_x.shape[0], imgA_u_w.shape[0]

            preds, preds_fp = model(torch.cat((imgA_x, imgA_u_w)), True)
            pred_x, pred_u_w = preds.split([num_lb, num_ulb])
            pred_u_w_fp = preds_fp[num_lb:]
 


            pred_u_s1, pred_u_s2 = model(torch.cat((imgA_u_s1, imgA_u_s2))).chunk(2)

            pred_u_s1, pred_u_s3 = model(torch.cat((imgA_u_s1, imgA_u_s3))).chunk(2)
...
############################################################################################
            pred_l = torch.cat([pred_u_w_mix, pred_u_s2], dim=0)
            pred_r = torch.cat([pred_u_w_mix, pred_u_s3], dim=0)
            _, max_l = torch.max(pred_l, dim=1)
            _, max_r = torch.max(pred_r, dim=1)
            max_l = max_l.long()
            max_r = max_r.long()
            cps_loss = criterion_l(pred_l, max_r) + criterion_l(pred_r, max_l)
            ################################################################################
            ...
            loss = (loss_x + loss_u_s1 * 0.2 + loss_u_s2 * 0.2 + loss_u_w_fp * 0.5 + cps_loss*0.1) / 2.0
        ...
        iou_class, overall_acc,mean_dice = evaluate(model, valloader, cfg)
        print(mean_dice,'##########################')
        ...
        if rank == 0:
            ...
            writer.add_scalar('eval/MeanDice', mean_dice[0], epoch)
