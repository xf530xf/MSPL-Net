...
def evaluate(model, loader, cfg):
    ...
    with torch.no_grad():
        for imgA, mask, id in loader:

            imgA = imgA.cuda()

            pred = model(imgA).argmax(dim=1)
...
#############################################################################################
            for cls in range(1, cfg['nclass']):
                    # inter = ((pred.cpu() == cls) * (mask1.cpu() == cls)).sum().item()
                    # union = (pred == cls).sum().item() + (mask == cls).sum().item()
                    dice_class[cls-1] += 2.0 * intersection / union
 ...
 ###################################################################
    dice_class = [dice * 100.0 / len(loader) for dice in dice_class]
    mean_dice = sum(dice_class) / len(dice_class)



    return iou_class, overall_acc,mean_dice
...
def main():
    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        for i, (imgA, mask) in enumerate(trainloader):

            imgA,  mask = imgA.cuda(), mask.cuda()
